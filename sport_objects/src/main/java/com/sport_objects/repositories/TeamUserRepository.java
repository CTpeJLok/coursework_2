...
public interface TeamUserRepository extends JpaRepository<TeamUser, Long> {
    ...
    @Query("SELECT t FROM TeamUser t WHERE CONCAT(LOWER(t.entryDate), ' ', LOWER(t.exitDate), " +
            "' ', t.user.id) LIKE %?1%")
    List<TeamUser> searchKeyword(String searchKeyword);


    @Query("SELECT t FROM TeamUser t WHERE t.team.id = ?1 AND CONCAT(LOWER(t.entryDate), ' ', " +
            "LOWER(t.exitDate), ' ', t.user.id) LIKE %?2%")
